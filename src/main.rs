...
use wr::{Exercise, ExerciseCollection};
use yansi::Paint;
...
fn seek_the_path(exercises: &ExerciseCollection) -> Result<TestOutcome, anyhow::Error> {
    ...
    for exercise in exercises.opened()? {
        let exercise_outcome = run_tests(&exercise.manifest_path(exercises.exercises_dir()));
        match exercise_outcome {
            TestOutcome::Success => {
                println!("{}", success_style().paint(format!("\t🚀 {}", &exercise)));
            }
            TestOutcome::Failure { details } => {
                println!("{}", failure_style().paint(format!("\t❌ {}", &exercise)));
...
fn print_opened_message(exercise: &Exercise, exercises_dir: &Path) {
    println!(
        "{} {}",
        next_style().paint("\n\tAhead of you lies"),
